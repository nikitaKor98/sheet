...
import { useCellPicker } from "hooks/useCellPicker";
...
function Cell(props: any) {
  ...
  const column = useSelector(
    (store: any) => store.table.columns[idCell.replace(/[^A-Z]/g, "")]
  );
  const row = useSelector(
    (store: any) => store.table.rows[idCell.replace(/\D/g, "")]
  );

  const dispatch = useDispatch();
  const { handleMouseDown: handelMouseDownHook } = useCellPicker();
  ...
  const handleMouseDown = (event: any) => {
    ...
    const handelMouseUp = () => {
      if (prop === "width")
        dispatch(
          updateColumns({
            [id]: { [prop]: endLine - startLine + widthOrHeight },
          })
        );
      if (prop === "height")
        dispatch(
          updateRows({ [id]: { [prop]: endLine - startLine + widthOrHeight } })
        );
  ...
  return (
    <div ref={cellRef} className="cell" id={idCell}>
      <div
        style={{
          width: `${
            column
              ? column.width
              : className.split("__")[1].split("_")[0] === "left"
              ? "100%"
              : cellWidth
          }px`,
      ...
      >
        {type === "web_change" ? (
          <input
            ...
            // // onMouseUp={(e) => dragAndDropCell(idCell, e.type)}
            // onClick={() => selectionCell(idCell)}
            onMouseDown={(event) => handelMouseDownHook(event, { id: idCell })}
          />
        ) : (
          idCell
        )}
