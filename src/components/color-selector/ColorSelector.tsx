...
const whiten = (color: any, ratio: any) =>
  Math.round(color + (255 - color) * ratio);
const adjustSaturation =
  ({ r, g, b }: any) =>
  (ratio: any, adjustmentFn: any) => {
    return {
...
const saturate = (rgb: any, e: any, ref: any) => {
  ...
  if (leftDistRatio < 0.5) {
    const whitenessRatio = (0.5 - leftDistRatio) / 0.5;
...
function ColorSelector(props: any) {
  ...
  const backgroundColorTemp = (swatchSliderRef.current &&
    findRgbFromMouse(
      {
        pageX: convertProcentsToPxs(sliderPosition, sliderWidth) + sliderLeft,
      },
      swatchSliderRef,
      spectrumRanges
    )) || { r: 255, g: 255, b: 255 };
  ...
  const handelMouseDown = (event: any) => {
    ...
    if (id === "swatchPointer" || id === "slider") {
      ...
      setSwatchPointer(
        convertPxsToProcents(
          event.pageX -
            (swatchSliderRef.current?.getBoundingClientRect().left || 0),
      ...
      const rgb2 = findRgbFromMouse(
        ...
        boxRef,
        [{ from: [...spectrum[0].from], to: [rgb.r, rgb.g, rgb.b] }]
      );
    ...
    const handelMouseMove = (event: any) => {
      if (id === "swatchPointer" || id === "slider") {
        ...
        const { left: rangeLeft, right: rangeRight } =
          swatchSliderRef.current?.getBoundingClientRect() || {
            left: 0,
            right: 0,
          };
      ...
      if (id === "boxPointer" || id === "box") {
        const rgb = findRgbFromMouse(event, boxRef, spectrum);
        const { left: boxLeftSide, right: boxRightSide } =
        ...
        const { top: boxTopSide, bottom: boxBottomSide } =
  ...
  return (
    <div className="color-selector">
          ...
          return (
            <li
              ...
              className={`color-selector__color-palette_item${
                value === color
                  ? " color-selector__color-palette_item_active"
                  : ""
              } `}
      ...
      {isOpen && (
        <div className="color-selector__color-custom">
          <div
            ref={boxRef}
            id="box"
          ...
          >
            <div
              ...
              style={{
                left: `${boxPointerLeft}% `,
                top: `${boxPointerTop}% `,
                backgroundColor: `rgb(${activColor.r}, ${activColor.g}, ${activColor.b})`,
              }}
              onMouseDown={handelMouseDown}
              className="color-selector__color-custom_box_pointer"
            ></div>
          </div>
          <div className="color-selector__color-custom_slider">
            <div
              ...
              className="color-selector__color-custom_slider_swatch"
            ></div>
            <div
              ref={swatchSliderRef}
              id="slider"
            ...
            >
              <div
                ...
                style={{
                  left: `${sliderPosition}% `,
                  backgroundColor: `rgb(${backgroundColorTemp!.r}, ${
                    backgroundColorTemp!.g
                  }, ${backgroundColorTemp!.b})`,
                }}
                onMouseDown={handleSliderMouseDown}
                className="color-selector__color-custom_slider_swatch-slider_pointer"
              ></div>
            </div>
          </div>
