...
const removeMatchingElements = (sourceArray: any, valuesToRemove: any) => {
  return valuesToRemove
    ? sourceArray.filter((item: string) => !valuesToRemove.includes(item))
    : sourceArray;
};
const getColumnIndex = (cell: string) =>
  LETTERS.indexOf(cell.replace(/\d+/g, ""));
  ...
  const selectionCell = useCallback(
    (id: string) => {
      ...
      if (controlPress && isSelected) {
        ...
        const newActive =
          id === selectedCells[selectedCells.length - 1]
            ? selectedCells[selectedCells.length - 2]
            : selectedCells[selectedCells.length - 1];

        setCacheCells(null);
        dispatch(
          selectCell({ activeCell: newActive, selectedCells: updatedSelection })
        );
        return;
      }
      ...
      if (controlPress && !shiftPress && !isSelected) {
        setCacheCells(null);
        dispatch(
          selectCell({ activeCell: id, selectedCells: [...selectedCells, id] })
        );
        return;
      }
      ...
      if (shiftPress && controlPress) {
        ...
        const merged = [
          ...res,
          ...removeMatchingElements(selectedCells, cacheCells),
          ...invisibleCellsIsNull,
        ];
        const unique = merged.sort();

        setInvisibleCells(
          unique.filter(
            (item: any, index: any, arr: any) => item === arr[index - 1]
          )
        );
